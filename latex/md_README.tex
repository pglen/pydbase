...
The test can be repeated with running the \textquotesingle{}time.\+sh\textquotesingle{} script file. Please note the the time.\+sh clears all files tests/$\ast$ for a fair test. \begin{DoxyVerb}    sqlite time test, 500 records ...
    ...
    sys 0m0.004s
\end{DoxyVerb}
 Please mind the fact that the sqlite engine has to do a lot of parsing which we skip doing; That is why pydbase is an order of magnitude faster ...\hypertarget{md_README_autotoc_md7}{}\doxysubsection{Saving more complex data}\label{md_README_autotoc_md7}
